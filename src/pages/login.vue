...
const { ready, start } = useTimeout(3000, { controls: true }, true)
...
<div class="flex flex-col items-center justify-center h-screen-nonav">
    ...
    <div class="flex items-center justify-center overflow-hidden bg-gray-200 rounded-lg shadow-2xl">
        ...
        <form @submit.prevent="logginIn" class="flex flex-col p-4 space-y-4">
            ...
            <div class="flex space-x-2">
                <button type="submit" @submit.prevent="logginIn" class="w-1/2 py-2 text-red-200 bg-red-600 rounded-lg">Login</button>
            <button  
            @click="signingUp" 
            class="w-1/2 py-2 text-orange-200 bg-orange-600 rounded-lg">
            Sign Up</button>
            </div>
        <button @click="google" class="flex justify-center py-2 bg-white rounded-lg hover:bg-gray-200"><img 
    ...
    <div
    v-if = "!ready && error" 
    class="absolute w-1/4 p-4 text-center text-red-800 transition-opacity bg-red-300 rounded-lg bottom-2 right-2">{{ error }}</div>
</div>
